...
def correlation_protection_rate(records_set, avatars_set, variable_list = None, worst_case_protection_rate = None, boundaries = 0.05):
    ...
    if worst_case_protection_rate is None:
         raise ValueError('worst_case_protection_rate', 'float within [0,100] needs to be specified')
    ...
    combination_records = combination_count(records_set, sensitive_variable = variable_list)
    ...
    for i in range(0,n):
        ...
        for c in range(0,v):
            if(unique_records.iloc[:, c].dtype == np.float64 or unique_records.iloc[:, c].dtype == np.int64):
                iteration_dataframe = iteration_dataframe.loc[(iteration_dataframe.iloc[:,c] > (unique_records.iloc[i,c])-(working_df.iloc[:,c].mean()*(boundaries))) & (iteration_dataframe.iloc[:,c] < (unique_records.iloc[i,c])+(working_df.iloc[:,c].mean()*(boundaries)))]
            else:
                iteration_dataframe = iteration_dataframe.loc[iteration_dataframe.iloc[:,c] == unique_records.iloc[i,c]]
