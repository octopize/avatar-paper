...
def inference_metrics(records_set, avatars_set, variable_known, target, k=1, ncp=None):
    ...
    if ncp > len(variable_known):
        print(
            "Warning - specifying ncp higher than number of variables known might lead to error if variables are continuous or with few modalities, please consider setting a lower number of ncp"
        )
    ...
    if all(item in records_set.columns.tolist() for item in variable_known) == False:
        raise ValueError(
            "variable_known", "variable_known must be in records_set.columns"
        )
    ...
    if is_numeric_dtype(avatar_target):
        ...
        # Compute metrics
        working["symetric_percentage_error"] = (
            abs(working["estimation"] - working[target])
            / (abs(working[target]) + abs(working["estimation"]))
            * 200
        )
