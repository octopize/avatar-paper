...
### Utility Hazard ratio comparison AIDS
```{r}
Hazard_Ratio <- c(0.49, 0.40, 0.59, 0.25)
ci_high <- c(0.63, 0.52, 0.76, 0.31)
ci_low <- c(0.39, 0.31, 0.46, 0.25)
type <- c('Original', 'Avatar', 'Synthpop', 'CTGAN')
p_value <- c(1.22e-8, 1.47e-11, 5.24e-5, 3.35e-31)

aids_results <- data.frame(Hazard_Ratio, ci_hight, ci_low, type, p_value)
aids_results$type_f = factor(aids_results$type, levels=c('CTGAN', 'Synthpop', 'Avatar','Original'))

ggplot(aids_results, aes(x = Hazard_Ratio, y = type_f, colour = type)) +
...
= 0.15) +
   ...
   #geom_text(aes(y = type_f, label = p_value, color = 'black'), x = 0.1, size = 4) +
   theme(text = element_text(size = 16))

```

### Generic utility metric comparison (PYTHON)
```{python}

# Need to import original + avatar + synthpop + CTGAN 
## IMPORTANT dfs need to have proper 'id' column 
# run data['id'] = data.index after import
# Need to specify categorical_val and continous_val upstream

#### AIDS

target = prepare_common_data_format(aids,cat_columns= categorical_val,num_columns=continous_val)
syn_a = prepare_common_data_format(avatars,cat_columns= categorical_val,num_columns=continous_val)
syn_s = prepare_common_data_format(synthpop,cat_columns= categorical_val,num_columns=continous_val)
sy_c = prepare_common_data_format(gan,cat_columns= categorical_val,num_columns=continous_val)


status_ava, results_ava, df_ava = compare(target, syn_a, metrics_to_return="privacy-tests")
status_synth, results_synth, df_synth = compare(target, syn_s, metrics_to_return="privacy-tests")
status_ctgan, results_ctgan, df_ctgan = compare(target, sy_c, metrics_to_return="privacy-tests")

dcr_ava = list(results_ava['DCR']['details']['syn'])
dcr_synthpop = list(results_synth['DCR']['details']['syn'])
dcr_ctgan = list(results_ctgan['DCR']['details']['syn'])
dcr_ori = list(results_ava['DCR']['details']['tgt'])
dc = (dcr_ava+dcr_synthpop+dcr_ctgan)
...
nndr_ava = list(results_ava['NNDR']['details']['syn'])
nndr_synthpop = list(results_synth['NNDR']['details']['syn'])
nndr_ctgan = list(results_ctgan['NNDR']['details']['syn'])
nndr_ori = list(results_ava['NNDR']['details']['tgt'])
nnd = (nndr_ava+nndr_synthpop+nndr_ctgan)

df_nnd = pd.DataFrame(zip(quantile, nnd, type), columns=['holdout_proportion', 'NNDR', 'type'])

## DCR
import seaborn as sns
sns.set_theme(style="whitegrid")
color_dict = {'Original' : '#fd934d', 'Avatar' : '#3bd6b0', 'Synthpop' : '#b03bd6', 'CTGAN' : '#4db7fd'}
sns.lineplot(data=df_nnd, x="holdout_proportion", y="NNDR", hue="type", palette = color_dict)

## NNDR
sns.set_theme(style="whitegrid")
color_dict = {'Original' : '#fd934d', 'Avatar' : '#3bd6b0', 'Synthpop' : '#b03bd6', 'CTGAN' : '#4db7fd'}
sns.lineplot(data=df_dcr, x="holdout_proportion", y="DCR", hue="type", palette = color_dict)

#### WBCD
target = prepare_common_data_format(wbcd,cat_columns= categorical_val,num_columns=continous_val)
syn_a = prepare_common_data_format(avatars,cat_columns= categorical_val,num_columns=continous_val)
syn_s = prepare_common_data_format(synthpop,cat_columns= categorical_val,num_columns=continous_val)
sy_c = prepare_common_data_format(gan,cat_columns= categorical_val,num_columns=continous_val)
...
### Necessary functions for metrics computation (PYTHON)
```{python}
