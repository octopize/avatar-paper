...
get_survival_plot <- function(survival_curve, survival_data, hazard_ratio_synthetic, hazard_ratio_original, names = c("Original", "Synthetic")) {
  ...
  survival_plot <- ggsurv$plot <- ggsurv$plot +
    theme_minimal() +
    scale_linetype_discrete(name = c("Data source"), labels = c("Original", names[2])) +
    ...
    geom_label(aes(x = 59.5, y = 0.58, label = paste0(
      ...
      "\nSynthpop: HR [CI: 95%] = ", formatC(hazard_ratio_synthetic[2], format = "f", digits = 2),
...
# Avatar
survival_results <- get_survival_results(data = data, synthetic = avatar, names = c("Original", "Avatar"))
# Display time: about 15 seconds
survival_data <- survival_results$data
survival_curve <- survival_results$curve
hazard_ratio_synthetic <- survival_results$hazard_ratio_synthetic
hazard_ratio_original <- survival_results$hazard_ratio_original

survival_plot_avatar <- get_survival_plot(survival_curve = survival_curve, survival_data = survival_data, hazard_ratio_synthetic = hazard_ratio_synthetic, hazard_ratio_original=hazard_ratio_original)



# Synthpop
survival_results <- get_survival_results(data = data, synthetic = synthpop, names = c("Original", "Synthpop"))
# Display time: about 15 seconds
survival_data <- survival_results$data
survival_curve <- survival_results$curve
hazard_ratio_synthetic <- survival_results$hazard_ratio_synthetic
hazard_ratio_original <- survival_results$hazard_ratio_original
survival_plot_synthpop <- get_survival_plot(survival_curve = survival_curve, survival_data = survival_data, hazard_ratio_synthetic = hazard_ratio_synthetic, hazard_ratio_original=hazard_ratio_original)

# ctgan
survival_results <- get_survival_results(data = data, synthetic = ctgan, names = c("Original", "CT-GAN"))
survival_data <- survival_results$data
survival_curve <- survival_results$curve
hazard_ratio_synthetic <- survival_results$hazard_ratio_synthetic
hazard_ratio_original <- survival_results$hazard_ratio_original

survival_plot_ctgan <- get_survival_plot(survival_curve = survival_curve, survival_data = survival_data, hazard_ratio_synthetic = hazard_ratio_synthetic, hazard_ratio_original=hazard_ratio_original)
...
# color_vector <- c(rep("#c14f4f", 2), rep(""["avatar", "color"], 98))
