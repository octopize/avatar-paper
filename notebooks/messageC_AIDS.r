...
py_run_string("from metrics.privacy_metrics.avatars_are_k_hit import avatars_are_k_hit")
py_run_string("from metrics.privacy_metrics.hidden_rate import hidden_rate")

file_data <- "../datasets/"
file_results <- "../figures/"
...
# ggsave(file="../../figures/aids_famd2D_k_small_high.svg", plot=plot, width=10, height=7, dpi = 320)
...
if (compute) {
  ## parallele computing (not required if you import results datasets)
  library(doParallel)
  library(parallel)
  library(foreach)
  ...
  cl <- makeCluster(P) # start parallel computing

  clusterExport(cl, c("write_pvalue", "res_df"))

  system.time(
    res_par <- clusterApply(cl, p, fun = parralele_function) # eval parralel_function across k
  )
...
# get stat smooth (2,3) coordinate in order to draw the polygon between the smooth curves
...
# ggsave(file = "../figures/aids_hazardratio_variousk.svg", plot = plot, width = 10, height = 7)
...
for (k in seq_k) {
  ...
  #  Subset and input the avatar dataset
...
# ggsave(file="../figures/aids_localCloaking_violinplot.svg", plot = plot, width = 10, height = 7)
