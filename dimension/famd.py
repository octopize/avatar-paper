...
class FAMD(DimensionReduction):
    ...
    def fit(self, X):
        ...
        self.is_quanti = X.select_dtypes(include=['int', 'float', 'number']).columns.values
        self.is_quali = X.select_dtypes(exclude=['int', 'float', 'number']).columns.values
        ...
        weight_quali_rep = list(chain.from_iterable(repeat(i, j) for i, j in zip(list(weight_quali.iloc[0]), modality_numbers)))
    ...
    def _FAMD_contrib(self, X_original, X, col_w, row_w):
        ...
        weightedTc = self._rmultiplication(self._rmultiplication(X.T,np.sqrt(col_w)).T,np.sqrt(row_w))
        ...
        coord = pd.DataFrame(self.U[:,:ncp0] * self.s[:ncp0], columns=self.columns[:ncp0])
        mods = []
        ...
        for count, col in enumerate(dfquali.columns):
            ...
            for j, coordcol in enumerate(coord.columns):
                ...
                for i in range(len(dummy.columns)):
                    s+=(np.array(dummy.T)[i]*coord[coordcol]*row_w).sum()**2/self.prop[fi+i]
                dim += [s]
            eta1 = (np.array(dim)/(np.array((coord**2)).T*row_w).sum(axis = 1)).tolist()
            eta2 += [eta1]
