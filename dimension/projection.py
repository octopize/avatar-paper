...
class Projection:  
    ...
    # select columns and project data 
        ...
        if len(self._is_quali) > 0:
            self._df_quali = pd.get_dummies(self._data_frame[self._is_quali].astype("category"))
            bind = pd.concat([self._df_quanti, self._df_quali], axis = 1)
        else :
            bind = self._df_quanti

        cor = pd.DataFrame({component: {
    ...
    # Transform date into time post epoch
        ...
        for i in range(0, data_frame.shape[1]):
            if data_frame.iloc[:, i].dtype == ('datetime64[ns]'):
                data_frame.iloc[:, i] = (data_frame.iloc[:, i] - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
                datetime_variables.append(i)
        self.datetime_variables = datetime_variables

    # check column types
        self._is_quanti = data_frame.select_dtypes(include=['int', 'float', 'number']).columns.values
        self._is_quali = data_frame.select_dtypes(exclude=['int', 'float', 'number']).columns.values
    ...
    # specify the correct model
        if self._is_quali.size == 0:
            print('Regarding variables types, a PCA is performed for dimension reduction')
            model = PCA(nf=nf, col_w = col_w)
        elif self._is_quanti.size == 0:
            print('Regarding variables types, a MCA is performed for dimension reduction')
            model = MCA(nf=nf, col_w = col_w, stats = stats)
        else:
            print('Regarding variables types, a FAMD is performed for dimension reduction')
    ...
    # compute projection stats
        if stats :
            ...
            if self._is_quali.size == 0:
                ...
                self.contrib = self.cos2.div(self.cos2.sum(axis=0), axis=1).applymap(lambda x : x*100)
            elif self._is_quanti.size == 0:
                self.cos2 = self.cor.applymap(lambda x : x**2)
                self.contrib = pd.DataFrame(self.model.contrib, columns = self.cor.columns, index = list(self.cor.index))
            else :
                self.cos2 = pd.DataFrame(self.model.cos2,  index = list(model.is_quanti) +list(model.is_quali))
                self.contrib = pd.DataFrame(self.model.contrib, columns = self.cor.columns, index = list(self.cor.index))
    ...
    def transform(self, X):
        ...
        for i in self.datetime_variables:
            X.iloc[:, i] = (X.iloc[:, i] - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
    ...
    def inverse_transform(self, coord, shuffle = False):
        ...
        if len(self._is_quanti) != 0 :
            ...
            for column in inverse_quanti.columns:
                inverse_quanti['decimals'] = self._data_frame[column].apply(self.decimal_count)
                #shuffling the decimals for the avatarization
                if shuffle :
                    inverse_quanti['decimals'] = np.random.permutation(inverse_quanti['decimals'].values)

                inverse_quanti[column] = inverse_quanti[[column, 'decimals']].apply(lambda x : np.round(x[column], int(x['decimals'])), axis = 1)
        ...
        if len(self._is_quali) != 0 :
            ...
            X_quali.columns = list(pd.get_dummies(self._data_frame [self._is_quali], prefix=['' for i in range(len(self._is_quali))], prefix_sep = '').columns)
            ...
            for i in range(len(modalities)):
                inverse_quali[list(self._data_frame[self._is_quali].columns)[i]] = X_quali.iloc[:,val:val+modalities[i]].idxmax(1)
                val += modalities[i]
        ...
        for i in self.datetime_variables:
            inverse.iloc[:, i] = (inverse.iloc[:, i] * np.timedelta64(1, 's')) + np.datetime64('1970-01-01T00:00:00Z')
    ...
    def plot_circle(self, dimensions = [1,2], min_cor = 0.1, max_var = 7):
        ...
        for name, row in cor.iterrows():
            if i < max_var and (np.abs(row[dimensions[0]-1]) > min_cor or np.abs(row[dimensions[1]-1]) > min_cor):
                x = row[dimensions[0]-1]
                y = row[dimensions[1]-1]
                plt.arrow(0.0, 0.0, x, y, color='k', length_includes_head=True, head_width=.05)
        ...
        plt.xlabel("Dim " + str(dimensions[0]) + " (%s%%)" %
                    str(explained_var_ratio[dimensions[0]-1]* 100)[:4],
                    fontsize=figure_axis_size * 2)
        plt.ylabel("Dim " + str(dimensions[1]) + " (%s%%)" %
                    str(explained_var_ratio[dimensions[1]-1 ] * 100)[:4],
                    fontsize=figure_axis_size * 2)
    ...
    def plot_explained_var(self, max_dims = 10):
        ...
        plt.xticks(range(len(explained_percentage)), range(1,len(explained_percentage)+1), rotation='horizontal')
