...
from lsg.dimension.famd import FAMD
from lsg.dimension.mca import MCA
from lsg.dimension.pca import PCA
...
class Projection:
    ...
    def fit_transform(
        self, df, nf: Optional[int] = None, col_w=None, stats=True
    ):
        """Project data into a lower dimensional space using PCA,MCA or FAMD.

        Arguments:
            df {array-like} -- matrix to project
            nf {int} -- number of components to keep
              (default: {min(df.shape[0], df.shape[1])})
            col_w {numeric list} -- allow to increase the importance of
              a variable to separate records on the first PC's
        ...
        # check column types
        self._is_quanti = df.select_dtypes(
            include=["int", "float", "number"]
        ).columns.values
        self._is_quali = df.select_dtypes(
    ...
    def transform(self, X):
        """Project the data into the fitted space.
